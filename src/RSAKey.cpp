...
std::string RSAKey::encryptWithLocalPrivate(const std::string& plaintext) {
    try {
        ...
        StringSource ss(plaintext, true, new SignerFilter(rng, signer, new StringSink(ciphertext)));
    ...
    try {
        ...
        StringSource ss(
            ...
            new SignatureVerificationFilter(
                verifier, new ArraySink(reinterpret_cast<byte*>(&result), sizeof(result)),
...
std::string RSAKey::base64Encode(const std::string& data) const {
    std::string encoded;
    StringSource ss(data, true, new Base64Encoder(new StringSink(encoded)));
    return encoded;
}
